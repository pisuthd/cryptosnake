function CManageCoins(oParentContainer, game_type) {
  var _aFoods;
  ...
  this.foodsInSections = function () {
    let max = MAX_FOODS_INSTANCE
    if (game_type == 'tutorial') {
      max = 20
    }
    var iFoodsForSection = Math.floor(max / FIELD_SECTION_SUBDIVISION.tot);
  ...
  this._spawnRandomFoods = function (oSection) {
    ...
    while (bCheckCol) {
      ...
      if (!bCheckCol) {
        ...
        var aSnakes = this.getGame().getSnakesArray();
  ...
  this.checkCollisionFoodToObject = function (oObj, iRadius, iXRand, iYRand) {
    if (this.getGame().circleToCircleCollision({
        x: iXRand,
        y: iYRand
      }, oObj.getPos(), iRadius, oObj.getDim().w)) {
      return true;
    }
    return false;
  };
