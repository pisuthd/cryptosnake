...
function CTutorial(oData) {
  ...
  this._init = function () {
    ...
    _oWebcamController = new CWebcamController();
  ...
  this.getHandAngle = function (event) {
    ...
    for (let r of event) {
      if (r.part == 'leftWrist') {
        //left_hand_pointer.graphics.beginFill("red").drawCircle(0, 0, 5);
        //left_hand_pointer.x = r.position.x * 3
        //left_hand_pointer.y = r.position.y * 3
        ...
        //s_oStage.addChild(left_hand_pointer);
        totalParts += 1
      } else if (r.part == 'rightWrist') {
        //right_hand_pointer.graphics.beginFill("green").drawCircle(0, 0, 5);
        // right_hand_pointer.x = r.position.x * 3
        //right_hand_pointer.y = r.position.y * 3
        ...
        //s_oStage.addChild(right_hand_pointer);
        totalParts += 1
      }
    }
  ...
  this.getHeadPos = function (event) {
    ...
    for (let r of event) {
      if (r.part == 'leftEye') {
        ...
        totalPos.x += (r.position.x * 3)
        totalPos.y += (r.position.y * 3)

        totalParts += 1
      } else if (r.part == 'rightEye') {
    ...
    if (totalParts != 0) {
      head_position = {
        x: totalPos.x / totalParts,
        y: (totalPos.y / totalParts)
      }
    }
  ...
  this.onMove = function (event) {
    ...
    let head_position = this.getHeadPos(event)
    //console.log(head_position, CANVAS_WIDTH * 0.25)
    //let hand_position = this.getHandPos(event);

    //face_pointer.graphics.beginFill("DeepSkyBlue").drawCircle(0, 0, 5);
    //face_pointer.x = head_position.x
    //face_pointer.y = head_position.y
    //s_oStage.addChild(face_pointer);
    ...
    if (this.hasHand(event)) {
      ...
      //if ((60 > angle > 5) || (-60 < angle < -5)) {
      //_oPlayerSnake.rotation(angle)
      //} else {
      // s_oTutorial.onKeyReleased();
      //}


      // angle in degrees
      //let angleDeg = angle(head_position.x, head_position.y, hand_position.x, hand_position.y)

      //console.log(head_position, hand_position, angleDeg)
      // 90 for left
      //_oPlayerSnake.setRotation(angleDeg)
    } else {
      s_oGame.onKeyReleased();
    }
    //console.log(head_position.x,hand_position.x)
    /*
    if ((hand_position.x-head_position.x)>margin) {

      s_oTutorial.onRight();
    } else if ((head_position.x-hand_position.x)>margin) {
      s_oTutorial.onLeft();
    } 
    else {
      s_oTutorial.onKeyReleased();
    ...
    */
    ...
    /*
    if (head_position.x < CANVAS_WIDTH * 0.25) {
      console.log('go left')
      s_oTutorial.onLeft();
    } else if (head_position.x > CANVAS_WIDTH - (CANVAS_WIDTH * 0.25)) {
