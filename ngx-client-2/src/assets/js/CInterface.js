function CInterface() {
  ...
  this._init = function () {
    ...
    _pStartPosScore = {
      x: (CANVAS_WIDTH / 2) - 550,
      y: 55
    };
    _oScoreText = new createjs.Text("Coin" + ": 0", "32px " + FONT_GAME, "#ffffff");
    ...
    _pStartPosPause = {
      x: _pStartPosExit.x - oSprite.height - 20,
      y: _pStartPosExit.y
    };
    ...
    */
    if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
      ...
      _pStartPosFullscreen = {
        x: _pStartPosAudio.x - oSprite.width / 2 - 20,
        y: _pStartPosExit.y
      };
    } else {
      _pStartPosFullscreen = {
        x: _pStartPosPause.x - oSprite.height - 20,
        y: _pStartPosExit.y
      };
    }
  ...
  this.refreshBestScore = function (iScore, bAnim) {
    ...
    if (bAnim) {
      ...
      createjs.Tween.get(_oBestScoreText, {
        override: true
      }).to({
          scaleX: 1.1,
          scaleY: 1.1
